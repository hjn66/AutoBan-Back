...
module.exports.createToken = async function(mobileNumber) {
  ...
  expired = await Utils.addMinute(
    new Date(),
    config.get("sms_token_expiration_min")
  );
  return await SMSToken.create({
...
module.exports.checkToken = async function(token) {
  ...
  smsToken = await SMSToken.findOne({
    where: {
      [Op.and]: [{ mobileNumber: token.mobileNumber }, { token: token.token }]
    }
  });
  if (smsToken) {
